...
async function copyConsoleLink(roleName: string, accountId: string, partition: string) {
    await writeText(await invoke("open_web_console", { partition, accountId, roleName }));
...
<template>
    <VCard height="100%" min-width="300px">
        ...
        <VCardSubtitle
            @click="snackbarText = snackMessages.EMAIL; snackbar = true; writeText($props.account.email_address)">{{
                $props.account.email_address }}
        </VCardSubtitle>
        <VCardSubtitle @click="snackbarText = snackMessages.ID; snackbar = true; writeText($props.account.account_id)">{{
                ...
                <td>
                    <VBtnGroup class="float-right py-2" rounded="1" divided>
                        <VBtn color="secondary"
                            ...
                            <VTooltip activator="parent" open-delay="300">Copy credentials to clipboard
                            </VTooltip>
                        </VBtn>
                        <VBtn color="secondary"
                            @click="copyConsoleLink(role.role_name, $props.account.account_id, $props.partitionSlug)">
                            <VIcon>mdi-link-plus</VIcon>
                            <VTooltip activator="parent" open-delay="300">Copy console link to clipboard
                            </VTooltip>
                        </VBtn>
                        <VBtn color="primary"
                            @click="openWebConsole(role.role_name, $props.account.account_id, $props.partitionSlug)">
                            <VIcon>mdi-open-in-new</VIcon>
                            <VTooltip activator="parent" open-delay="300">Open in web console</VTooltip>
                        </VBtn>
                    </VBtnGroup>
                </td>
            </tr>
